...
- 2026-09-11: **epoc-x case studies wired.** Added `trackCaseStudiesEpocX` (`content_click` + `casestudies` + `product=epoc_x`), plant on `Case Study` card component root ×3. Audit found mid-page content UNtracked: testimonials/features skipped (no CTA, nothing clickable), leadmagnet skipped (display card only — no form in DOM, no `form_submit` possible). Hero ×2 + productnav ×6 + accessories ×6 remain the only page-local tracked sections.
