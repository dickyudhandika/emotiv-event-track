...
- 2026-09-11: **epoc-x case studies + specs wired.** Added `trackCaseStudiesEpocX` (`content_click` + `casestudies` + `product=epoc_x`, plant on `Case Study` card root ×3) and `trackSpecsEpocX` (`content_click` + `specs` + `product=epoc_x`, plant on BOTH `spec - desktop - black` and `spec - mobile - black` — the static spec table has no links/buttons, so section-root click = reader-engagement signal). Audit found mid-page content UNtracked: testimonials/features skipped (no CTA, nothing clickable), leadmagnet skipped (display card only — no form in DOM, no `form_submit` possible). Hero ×2 + productnav ×6 + accessories ×6 + case studies ×3 remain the click-tracked page-local sections.
